...
class Wikipedia(API):
    ...
    def search_geo(self,lat=48.858844,lon=2.294351,radius=1000,limit=10):
        url = self._api_url+'list=geosearch&gsradius='+str(radius)+'&gscoord='+str(lat)+'|'+str(lon)+'&gslimit='+str(limit)
        data = self._tools.data_from_url(url)
        self._increment_nb_call()
